...
class ESMForAffinity(nn.Module):
    ...
    - Uses Gradient Checkpointing (use_reentrant=False) for massive memory savings.
    """
    def __init__(
        self,
        ...
        # CRITICAL FIX for Meta-Learning OOM: Enable gradient checkpointing 
        # MUST set use_reentrant=False to support create_graph=True (2nd order gradients)
        self.esm.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
...
def train_datarater(
    ...
    """
    Meta-learn DataRater weights for samples using Bilevel Optimization.
    Incorporates BF16 Mixed Precision for memory efficiency while preserving FP32 loss stability.
    """
    from torch.backends.cuda import sdp_kernel
    
    # Enforce Math SDPA to prevent C++ kernel crashes during 2nd-order differentiation
    ...
    try:
        ...
        for step in range(n_meta_steps):
            ...
            for m_idx in models_to_process:
                ...
                for _t in range(T_window):
                    ...
                    # AMP: Compute forward pass in bfloat16 to save memory
                    ...
                    # Calculate loss in float32 for perfect gradient stability
                ...
                outer_per = F.mse_loss(outer_preds.float(), out_targets.float(), reduction="none")

                if use_first_order_ablation:
                    with autocast(device_type="cuda", dtype=torch.bfloat16):
                        out_scores = data_rater(out_ids, out_mask)
                        out_w = F.softmax(out_scores / tau, dim=0)
                    outer_loss = torch.sum(out_w.float() * outer_per)
                else:
        ...
        # Always restore SDPA backend state safely
...
def filter_dataset(data_rater, original_dataset, N_ref=10000, B=256, keep_ratio=0.7):
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            # Use BF16 for fast inference scoring
            with autocast(device_type="cuda", dtype=torch.bfloat16):
